...
if __name__ == '__main__':
    ...
    for f in conf_files:
        ...
        for run_id, run in enumerate(cluster_runs):
            stats[int(run_id)] = list()
            # train model on each cluster
            for cluster_id, cluster in run.items():
                print('____________________________________________________')
                print(f'Running {f.stem} run {run_id} cluster {cluster_id}')
                try:
                    ...
                    print(f"Random Search - {f.stem} : run{run_id} : {cluster_id}")
                    # print(best_model_r.summary())
                    print(f'loss: {loss_r} | mse: {mse_r}')
                    
                    best_model_r.save(out_dir / f'{f.stem}_run{run_id}_{cluster_id}.h5')
                    ...
                    print(f'Error in {f.stem}_run{run_id}_c{cluster_id}', e)
                    exit(1)