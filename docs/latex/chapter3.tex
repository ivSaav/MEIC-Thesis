...
\begin{table}[]
    ...
    \multirow{4}{*}{Inclusion}    & I1          & The document focuses on clustering algorithms, evaluation metrics or methods.                                                                                                                                                   \\
                                  ...
                                  & I4          & \begin{tabular}[c]{@{}l@{}}Provides a comparison between the method and other clustering approaches\\  or analyses multiple clustering approaches for a given application.\end{tabular}                                         \\
                                  & I5          & \begin{tabular}[c]{@{}l@{}}The article focuses on the application of clustering methods to improve\\  the performance of machine learning approaches.\end{tabular}                                                              \\ \hline
    \multicolumn{1}{c}{Exclusion} & E1          & \begin{tabular}[c]{@{}l@{}}The article doesn't cover clustering. This can occur due to the ambiguity of the term "cluster", \\ as it can also be used in biology to talk about gene clusters.\end{tabular}                      \\
    \multicolumn{1}{c}{}          & E2          & The paper does not clearly evaluate the results of the approach.                                                                                                                                                                \\
    \multicolumn{1}{c}{}          & E3          & \begin{tabular}[c]{@{}l@{}}The number of recorded citations is less than 50. As clustering is a well-developed study, \\ the criteria for the relevancy of the paper can be more restrictive than on other fields.\end{tabular} \\
                                  & E4          & \begin{tabular}[c]{@{}l@{}}The paper covers clustering algorithms for unsuited data types. Articles that pertain \\ to the use of clustering techniques on multi-view data are excluded.\end{tabular}                           \\
...
% \begin{description}
%     \item[Mousavi et al.]\cite{Mousavi.Horton.ea_Seismicfeaturesautomatic_2016} propose employing spectral clustering along with logistic regression and ANN models to automatically distinguish between deep and shallow microseismic events (low-amplitude events, attributed to the reduction in effective stress). The authors point out that there already exist several methods capable of distinguishing between the two types of events, but they are not automatic and require human intervention in the form of generating class labels for training purposes. Therefore, the goal of this study is to develop a system that can automatically perform the task.
    
%     The feasibility of the method is tested on microearthquakes gathered from a salt mine collapse in Louisiana. The hierarchical clustering algorithm is applied to the data to determine if there was a clear separation between the two types of events. The results showed that there was some concentration of deep events on the edges, but overall the data was not separable. 
% \end{description}
...
\begin{table}[!ht]
    \caption{List of reviewed papers for the clustering SLR.}
\label{tab:clustering_slr_results}
    \begin{adjustwidth}{-1.5cm}{}
        \resizebox{0.90\textwidth}{!}{\begin{minipage}{\textwidth}
    \centering
    \begin{tabular}{@{}cccccc@{}}
    \toprule
    \textbf{Paper}   & \textbf{Year} & \textbf{Type} & \textbf{Application}                   & \textbf{Objective}              & \textbf{DR} \\ \midrule
    Jain \cite{Jain_Dataclustering50_2010}            & 2010          & Survey        & N/A                                    & Cluster Algo. + Analysis & No          \\ \midrule
    Rodriguez et al. \cite{Rodriguez.Laio_Clusteringfastsearch_2014} & 2014          & Primary       & General                                & Cluster Algo.                   & No          \\ \midrule
    Fahad et al. \cite{Fahad.Alshatri.ea_SurveyClusteringAlgorithms_2014a}    & 2014          & Survey        & High Dimension Datasets                & Cluster Algo. + Large Data      & Yes         \\ \midrule
    Kou et al. \cite{Kou20141}      & 2014          & Primary       & Financial Risk Assessment              & Cluster Decision Method         & No          \\ \midrule
    CAN \cite{Nie.Wang.ea_Clusteringprojectedclustering_2014a}             & 2014          & Primary       & General                                & Cluster Algo + Large Data       & Yes         \\ \midrule
    Granato et al. \cite{Granato.Santos.ea_Useprincipalcomponent_2018}  & 2018          & Review        & Bioactive Compound Identification      & Method Assessment               & Yes         \\ \midrule
    Lin et al. \cite{Lin.Tsai.ea_Clusteringbasedundersamplingclassimbalanced_2017}      & 2017          & Primary       & General                                & Improve ML (Sampling)           & No          \\ \midrule
    Douzas et al. \cite{Douzas.Bacao.ea_Improvingimbalancedlearning_2018}   & 2018          & Primary       & General                                & Improve ML (Sampling)           & Yes         \\ \midrule
...
The second search, performed in Google Scholar, yielded slightly better results than the previous one with more recent articles about the application of clustering techniques to machine learning tasks. The results were sorted in the function of "relevancy" which for the chosen platform indicates the best match between each document and the search terms. As a consequence, the first results weren't as skewed to older documents as in the first query.
...
\section{Analysis}\label{sec:gan_res_analysis}
...
\def\arraystretch{1.5}
\begin{table}[h]
\caption{List of reviewed papers for Adversarial Outlier Detection.}
...
\subsection{Threats to SLR Validity}\label{sec:gan_slr_threats}
